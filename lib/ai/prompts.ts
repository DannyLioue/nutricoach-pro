...
/**
 * 运动截图分析 Prompt
 * 用于分析运动APP（如GARMIN、Keep等）的截图，提取运动数据
 */
...
3. **强度**（intensity）:
   - 根据运动数据判断：低、中、高
   - 参考标准：
     - 低强度：心率 < 120 bpm，配速 > 7:00/km，RPE < 12
     - 中强度：心率 120-150 bpm，配速 5:00-7:00/km，RPE 12-15
     - 高强度：心率 > 150 bpm，配速 < 5:00/km，RPE > 15

4. **距离**（distance，可选）:
   - 如果截图显示距离（公里、米），以公里为单位
...
8. **描述**（description）:
   - 简要描述运动内容
   - 格式："[运动类型] [时长]，例如：'跑步30分钟'或'力量训练45分钟'"

**返回JSON格式（不要有markdown格式）：**

{
  "exerciseType": "运动类型（如：跑步、骑行、游泳、力量训练等）",
  "duration": {
    "minutes": 时长（数字，单位：分钟）
  },
  "intensity": "低/中/高",
  "distance": 距离（数字，单位：公里，可选）,
  "calories": 消耗卡路里（数字，可选）,
  "heartRate": {
    "average": 平均心率（数字，可选）,
    "max": 最大心率（数字，可选）
  },
  "pace": {
    "minKm": "配速（如：5'30''/km，可选）",
    "speed": "速度（如：10.5km/h，可选）"
  },
  "description": "运动描述（如：'跑步30分钟，平均心率145bpm'）"
}

**识别要点：**
1. 如果截图中有多个运动项目，提取主要项目或全部列出
2. 时长优先识别"时长"、"时间"、"用时"等字段
3. 如果截图不清晰或无法识别某些数据，对应字段设为 null
4. 距离单位可能是 km、公里、米，需要统一转换为公里
5. 卡路里可能是 kcal、千卡、卡，需要识别
